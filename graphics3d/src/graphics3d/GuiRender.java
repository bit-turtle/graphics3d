...
public class GuiRender {
    ...
    private void createUniforms() {
        uniformsMap = new UniformMap(shaderProgram.getId());
        uniformsMap.createUniform("scale");
        scale = new Vector2f();
    }
    
    private void setupKeyCallBack(Window window) {
        prevKeyCallBack = glfwSetKeyCallback(window.getHandle(), (_, key, _, action, _) -> {
            window.getEngine().keyCallback(key, action);
        ...
        glfwSetCharCallback(window.getHandle(), (_, c) -> {
            ImGuiIO io = ImGui.getIO();
            if (!io.getWantCaptureKeyboard()) {
                return;
            }
    ...
    public void render(Scene scene) {
        GuiInterface guiInstance = scene.getGui();
        if (guiInstance == null) {
            return;
        }
        guiInstance.drawGui();
